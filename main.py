...
def save_roi(frame, obj_id, elapsed_time):
    ...
    if frame.size > 0:  # ROI가 비어있지 않을 경우에만 저장
        ...
        if saved_image is not None:
            # 이미지에 파란색 테두리 추가 (테두리 크기 5픽셀)
            height, width, _ = saved_image.shape
            cv2.rectangle(saved_image, (0, 0), (width, height), (255, 0, 0), 15)  # (255, 0, 0) -> 파란색, 두께 5픽셀

            # 테두리가 추가된 이미지를 다시 저장
            cv2.imwrite(file_name, saved_image)
            print(f"파란색 테두리가 추가된 이미지 저장: {file_name}")
        ...
        for x1, y1, w, h, obj_id in results_tracker:
            ...
            if obj_id in tracking_times and "elapsed_time" in tracking_times[obj_id]:
                ...
                if elapsed_time <= 0.55:
                    # ROI 저장 시, 좌표 정보도 함께 전달
                    save_roi(frame, obj_id, elapsed_time)
                cvzone.putTextRect(frame, f'Time: {elapsed_time:.2f}s', (x1, y1 - 20), scale=1, thickness=1, offset=3, colorR=(255,0, 0))
